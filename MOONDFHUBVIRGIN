--[[
    ================================================================================
    MOONDF HUB - VERSÃO FINAL (MODIFICADA: TEMA PRETO + BRANCO CORRIGIDOS + DF ICON)
    ================================================================================
]]

-- ==============================================================================
--  CONFIGURAÇÃO DA FOTO (REMOVIDA - AGORA É TEXTO "DF")
-- ==============================================================================

-- A imagem foi substituída por texto no código abaixo, conforme solicitado.
...
local TRANSLATIONS = {
    PT = {
        ...
        EXECUTE_INNER = "Executar Rápido",
        EXECUTE_INNER_DESC = "Clique para executar (fecha grupo)",
    ...
    EN = {
        ...
        EXECUTE_INNER = "Quick Execute",
        EXECUTE_INNER_DESC = "Click to run (close group)",
...
local miniButton = new("TextButton", { 
    Name = "MiniButton", Parent = screenGui,
...
local function createEntry(params, parentFrame, depth, onChildrenChanged, _collapseFunc)
    depth = depth or 0
    local baseH = 60
    ...
    local function buildChildren(list)
        ...
        for _, childParam in ipairs(list or {}) do
            -- Pass collapse as extra arg downstream for executeImmediate
    ...
    elseif params.Type == "Single" then
        table.insert(connections, hoverBox.MouseButton1Click:Connect(function()
            if hubVisible then
                ...
                -- [ADICIONADO] Se foi chamado dentro de collapseFunc (foi passado), fecha grupo do botão
                if _collapseFunc and type(_collapseFunc) == "function" then
                    _collapseFunc()
                end
            end
        end))
...
function initTopics()
    ...
    -- TÓPICO: TESTE
    -- =========================
    addTopic(T("TOPIC_TEST"), {
        { Type = "Label", Name = T("DEMO_LABEL"), Description = T("DEMO_DESC") },
        { Type = "Single", Name = T("BTN_HITBOX"), Description = T("BTN_HITBOX_DESC"), Callback = function() print("Clicado!") end },
        { Type = "Toggle", Name = T("TOGGLE_HITBOX"), Description = T("TOGGLE_HITBOX_DESC"), StateKey = "TESTE_TOGGLE" },
        { Type = "Slider", Name = T("SLIDER_BLUE"), Description = T("SLIDER_BLUE_DESC"), Min = 0, Max = 100, Default = 50 },
        { Type = "InputImmediate", Name = T("INPUT_OK"), Description = T("INPUT_OK_DESC"), Placeholder = T("INPUT_OK_PLACE") },
        { Type = "InputSelect", Name = T("INPUT_NO_OK"), Description = T("INPUT_NO_OK_DESC"), Placeholder = T("INPUT_NO_OK_PLACE") },
        { Type = "ListPersistent", Name = T("DROPDOWN"), Description = T("DROPDOWN_DESC"), Options = {"Opção A", "Opção B", "Opção C"} },
        { Type = "ListAuto", Name = T("LIST_GROUP"), Description = T("LIST_GROUP_DESC"),
            Options = {
                { Type = "Toggle", Name = T("ITEM_INNER_1"), Description = T("ITEM_INNER_DESC_1") },
                { Type = "Slider", Name = T("ITEM_INNER_2"), Description = T("ITEM_INNER_DESC_2") },
                -- NOVO BOTÃO DE EXECUÇÃO IMEDIATA
                {
                    Type = "Single",
                    Name = T("EXECUTE_INNER"),
                    Description = T("EXECUTE_INNER_DESC"),
                    Callback = function()
                        print("Executado botão de execução rápida dentro do grupo!")
                    end
                }
            }
        },
        { Type = "ListAuto", Name = T("NEST_TEST"), Description = T("NEST_TEST_DESC"),
            Options = {
                { Type = "Label", Name = T("LEVEL_1"), Description = T("LEVEL_1_DESC") },
                { Type = "ListAuto", Name = T("SUB_LIST"), Description = T("SUB_LIST_DESC"),
                    Options = {
                        { Type = "Single", Name = T("BTN_LEVEL_3"), Description = T("BTN_LEVEL_3_DESC") },
                        { Type = "Toggle", Name = T("TOGGLE_LEVEL_3"), Description = T("TOGGLE_LEVEL_3_DESC") }
                    }
                }
            }
        }
